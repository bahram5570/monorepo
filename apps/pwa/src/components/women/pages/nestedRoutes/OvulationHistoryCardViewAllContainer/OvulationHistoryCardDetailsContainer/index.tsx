...
import { OvulationHistoryCardDetailsContainerProps } from './types';

const OvulationHistoryCardDetailsContainer = ({ detailsIndex }: OvulationHistoryCardDetailsContainerProps) => {
          ...
          <div className="w-full flex flex-col items-center px-4">
            ...
            <div className="w-full flex flex-col items-center gap-2">
              {detailsList.map((item, index) => (
                <TestGenerator createTime={item.createTime} result={item.result} key={index} />
              ))}
            </div>
          </div>
        )}
...
export default OvulationHistoryCardDetailsContainer;
