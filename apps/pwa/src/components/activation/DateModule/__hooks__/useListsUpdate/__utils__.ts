...
export const startAndEndDates: StartAndEndDatesTypes = ({ startDate, endDate }) => {
  const startList = startDate.split(DATE_SEPERATOR_REGEX);
  const endList = endDate.split(DATE_SEPERATOR_REGEX);
...
export const daysListUpdater: DaysListUpdaterTypes = (props) => {
  ...
  switch (props.calendarType) {
    case CalendarTypeEnum.Jalali:
      lastDayOfMonth = m.jDaysInMonth(Number(props.year), Number(props.month) - 1);
      break;

    case CalendarTypeEnum.Gregorian:
      lastDayOfMonth = m(`${props.year}-${props.month}`, 'YYYY-MM-DD').daysInMonth();
      break;
